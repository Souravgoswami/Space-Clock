...
def main()
	set title: 'About Space::Clock', background: 'black', width: $width, height: $height, resizable: true
	...
	l, s, stars = 0, {}, {}
	10.times do |temp|
		s.merge! temp => c = Square.new(x: rand(0..$width), y: rand($height .. $height + 1500), z: -5, size: rand(60..90)) ; c.opacity = rand(0.1..0.3)
	end

	50.times do |temp|
		tempsize = rand(8..15)
		stars[temp] =Image.new(path: ['crystals/hoverstars.png', 'crystals/hoverstars1.png', 'crystals/hoverstars2.png', 'crystals/hoverstars3.png',
	...
	($width/30).times do |temp|
		tempsize = rand(20..50)
		chash[temp] = Image.new(path: 'crystals/crystal3.png', x: rand(0..$width), y: $height - tempsize - 29, z: 1, width: tempsize, height: tempsize,color: '#232323')
	end
	moon, mooncolor, moonbool = Image.new(path: 'crystals/moon.png', width: 100, height: 100, y: $height - 100),	mooncolor = [1,1,1,1], false
	blackhole = Image.new(path: 'crystals/blackhole.png', x: rand(100..$width - 100), y: rand(100..$height - 100), z: -10,
		color: %w(orange blue fuchsia purple yellow green).sample)

	text = {}
	for line in file
		text[l] = Text.new font: 'mage/MateSC-Regular.ttf', text: line.chomp, x: 5, y: l * 21, color: 'white', size: 17
		l += 1
	end

	i, block = -1, {}
	($width/76).times do |temp|
		block[temp] = Image.new x: i * 89, y: $height - 30, path: 'crystals/block.png', width: 89
		i += 1
	end
	...
	on :mouse_move do |e|
		for val in s.values do val.opacity = 0 if val.contains?(e.x, e.y) end
	...
	update do
		...
		if Time.now.strftime("%N")[0].to_i % 2 == 0
			stars.values.sample.opacity = [0, 1].sample
